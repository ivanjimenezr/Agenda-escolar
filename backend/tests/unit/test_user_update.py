...
from src.application.schemas.user import PasswordChangeRequest, UserUpdateRequest
...
class TestUserUpdate:
    ...
    def test_update_user_name_only(self, db_session: Session, sample_user: User, user_use_cases: UserUseCases):
        ...
        update_data = UserUpdateRequest(name="Updated Name")

        updated = user_use_cases.update_user(user_id=sample_user.id, data=update_data)
    ...
    def test_update_user_name_and_email(self, db_session: Session, sample_user: User, user_use_cases: UserUseCases):
        ...
        update_data = UserUpdateRequest(name="New Name", email="newuser@example.com")
    ...
    def test_update_user_password_via_update_request(
        self, db_session: Session, sample_user: User, user_use_cases: UserUseCases
    ):
        ...
        update_data = UserUpdateRequest(current_password="password123", new_password="NewSecurePassword456")
    ...
    def test_update_user_password_wrong_current(
        ...
        update_data = UserUpdateRequest(current_password="wrongpassword", new_password="NewPassword")

        with pytest.raises(ValueError, match="Current password is incorrect"):
            user_use_cases.update_user(user_id=sample_user.id, data=update_data)

    def test_update_user_password_too_short(self, db_session: Session, sample_user: User):
        """Test password validation - too short"""
        with pytest.raises(ValueError):
            UserUpdateRequest(current_password="password123", new_password="short")  # Less than 8 characters
    ...
    def test_update_user_partial_password_fails(
        self, db_session: Session, sample_user: User, user_use_cases: UserUseCases
    ):
    ...
    def test_update_user_partial_password_reverse_fails(
        self, db_session: Session, sample_user: User, user_use_cases: UserUseCases
    ):
    ...
    def test_update_user_email_duplicate(self, db_session: Session, sample_user: User, user_use_cases: UserUseCases):
        ...
        # Create another user
        other_user = User(email="existing@example.com", name="Other User", password_hash="hashed")
        db_session.add(other_user)
        db_session.commit()

        # Try to update to existing email
        update_data = UserUpdateRequest(email="existing@example.com")
    ...
    def test_update_user_repository_level(self, db_session: Session, sample_user: User):
        ...
        updated = repo.update(user_id=sample_user.id, name="Updated via Repo", email="repoupdate@example.com")
    ...
    def test_update_nonexistent_user(self, db_session: Session, user_use_cases: UserUseCases):
        ...
        with pytest.raises(ValueError, match="User not found"):
            user_use_cases.update_user(user_id=fake_id, data=update_data)
...
class TestPasswordChange:
    ...
    def test_change_password_success(self, db_session: Session, sample_user: User, user_use_cases: UserUseCases):
        ...
        password_data = PasswordChangeRequest(current_password="password123", new_password="NewSecurePass789")

        result = user_use_cases.change_password(user_id=sample_user.id, data=password_data)
    ...
    def test_change_password_wrong_current(self, db_session: Session, sample_user: User, user_use_cases: UserUseCases):
        ...
        password_data = PasswordChangeRequest(current_password="wrongpassword", new_password="NewPassword123")

        with pytest.raises(ValueError, match="Current password is incorrect"):
            user_use_cases.change_password(user_id=sample_user.id, data=password_data)

    def test_change_password_validation_too_short(self, db_session: Session):
        """Test password validation in PasswordChangeRequest"""
        with pytest.raises(ValueError):
            PasswordChangeRequest(current_password="oldpass", new_password="short")  # Less than 8 characters

    def test_change_password_same_as_current(
        self, db_session: Session, sample_user: User, user_use_cases: UserUseCases
    ):
        """Test changing password to same value (should work but is discouraged)"""
        password_data = PasswordChangeRequest(current_password="password123", new_password="password123")
