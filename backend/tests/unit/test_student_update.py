...
def sample_user(db_session: Session):
    ...
    user = User(email="teststudentupdate@example.com", name="Test Student Update User", password_hash="hashed_password")
    db_session.add(user)
    db_session.commit()
...
class TestStudentUpdate:
    ...
    def test_update_student_basic_fields(
        self, db_session: Session, sample_user: User, student_use_cases: StudentUseCases
    ):
        ...
        # Update basic fields
        update_data = StudentUpdateRequest(name="Updated Name", school="Updated School", grade="6th Grade")

        updated = student_use_cases.update_student(student_id=student.id, user_id=sample_user.id, data=update_data)
    ...
    def test_update_student_allergies(self, db_session: Session, sample_user: User, student_use_cases: StudentUseCases):
        ...
        # Update allergies
        update_data = StudentUpdateRequest(allergies=["gluten", "lactose", "peanuts"])
    ...
    def test_update_student_excluded_foods(
        self, db_session: Session, sample_user: User, student_use_cases: StudentUseCases
    ):
        ...
        update_data = StudentUpdateRequest(excluded_foods=["spinach", "broccoli", "mushrooms"])
    ...
    def test_update_student_clear_allergies(
        self, db_session: Session, sample_user: User, student_use_cases: StudentUseCases
    ):
        ...
        # Clear allergies
        update_data = StudentUpdateRequest(allergies=[])

        updated = student_use_cases.update_student(student_id=student.id, user_id=sample_user.id, data=update_data)

        assert updated.allergies == []

    def test_update_student_avatar_url(
        self, db_session: Session, sample_user: User, student_use_cases: StudentUseCases
    ):
        ...
        # Add avatar
        update_data = StudentUpdateRequest(avatar_url="https://example.com/avatar.jpg")

        updated = student_use_cases.update_student(student_id=student.id, user_id=sample_user.id, data=update_data)

        assert updated.avatar_url == "https://example.com/avatar.jpg"

    def test_update_student_remove_avatar(
        self, db_session: Session, sample_user: User, student_use_cases: StudentUseCases
    ):
        ...
        # Remove avatar
        update_data = StudentUpdateRequest(avatar_url=None)

        updated = student_use_cases.update_student(student_id=student.id, user_id=sample_user.id, data=update_data)

        assert updated.avatar_url is None

    def test_update_student_all_fields(
        self, db_session: Session, sample_user: User, student_use_cases: StudentUseCases
    ):
    ...
    def test_update_student_partial(self, db_session: Session, sample_user: User, student_use_cases: StudentUseCases):
        ...
        # Update only name
        update_data = StudentUpdateRequest(name="Updated Name Only")
    ...
    def test_update_student_empty_lists(
        self, db_session: Session, sample_user: User, student_use_cases: StudentUseCases
    ):
        ...
        update_data = StudentUpdateRequest(allergies=[], excluded_foods=[])
    ...
    def test_update_student_schema_validation(self, db_session: Session):
        ...
        # Valid update
        valid = StudentUpdateRequest(name="Valid Name", school="Valid School", grade="5th Grade")
        ...
        # Lists
        with_lists = StudentUpdateRequest(allergies=["gluten", "lactose"], excluded_foods=["nuts", "shellfish"])
    ...
    def test_update_student_repository_level(self, db_session: Session, sample_user: User):
        ...
        student = StudentProfile(
            user_id=sample_user.id, name="Original", school="School", grade="5th", allergies=[], excluded_foods=[]
        )
        db_session.add(student)
        db_session.commit()
        db_session.refresh(student)
        ...
        updated = repo.update(student_id=student.id, name="Updated via Repo", grade="6th")
    ...
    def test_update_nonexistent_student(
        ...
        update_data = StudentUpdateRequest(name="Should Fail")

        with pytest.raises(ValueError, match="Student not found"):
            student_use_cases.update_student(student_id=fake_id, user_id=sample_user.id, data=update_data)
    ...
    def test_update_student_different_user(self, db_session: Session, student_use_cases: StudentUseCases):
        ...
        user1 = User(email="user1@example.com", name="User 1", password_hash="hash")
        db_session.add(user1)
        db_session.commit()
        db_session.refresh(user1)

        student = StudentProfile(
            user_id=user1.id, name="Student", school="School", grade="5th", allergies=[], excluded_foods=[]
        )
        db_session.add(student)
        db_session.commit()
        db_session.refresh(student)
        ...
        # Create second user
        user2 = User(email="user2@example.com", name="User 2", password_hash="hash")
        ...
        with pytest.raises(PermissionError, match="Access denied"):
            student_use_cases.update_student(student_id=student.id, user_id=user2.id, data=update_data)
