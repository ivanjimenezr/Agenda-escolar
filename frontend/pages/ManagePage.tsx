        ...
        {availableTabs.map((key) => (
          <button
            ...
            className={`px-4 py-2 text-xs font-bold rounded-xl transition-all whitespace-nowrap ${
              activeTab === key ? 'bg-blue-600 text-white shadow-sm' : 'bg-white dark:bg-gray-800 text-gray-500 font-bold'
            }`}
          >
      ...
      <div className="space-y-3">
        {(dataMap[activeTab].data as Item[]).map(item => (
            <div key={item.id} className="bg-white dark:bg-gray-800 rounded-xl p-4 flex justify-between items-center shadow-sm border border-gray-100 dark:border-gray-700">
                ...
                <div className="flex space-x-1">
                    <button onClick={() => openModal(item)} className="p-2 text-blue-500 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                        <PencilIcon className="w-4 h-4" />
                    </button>
                    <button onClick={() => { if(confirm('¿Borrar?')) dataMap[activeTab].setData((prev: any[]) => (prev as any).filter((p: any) => p.id !== item.id)) }} className="p-2 text-red-500 bg-red-50 dark:bg-red-900/20 rounded-lg">
        ...
        {(dataMap[activeTab].data as Item[]).length === 0 && (
            <div className="text-center py-12 text-gray-400">
                <p className="text-sm font-medium">No hay datos para esta sección.</p>
                <p className="text-xs">Pulsa + para añadir.</p>
            </div>
        )}
      </div>

      <button onClick={() => openModal()} className="fixed bottom-20 right-6 bg-blue-600 text-white rounded-2xl p-4 shadow-lg active:scale-90 z-20">
...
const ItemDisplay: React.FC<{item: Item, type: Manageable, centers: Center[]}> = ({ item, type, centers }) => {
    switch(type) {
        case 'subjects':
            ...
            const daysDisplay = s.days ? s.days.map(d => d.substring(0, 3)).join(', ') : '';
            return <div><p className="font-bold text-sm">{s.name}</p><p className={`text-[10px] font-bold uppercase ${s.type === 'extraescolar' ? 'text-purple-500' : 'text-blue-500'}`}>{daysDisplay} • {s.time} • {s.type}</p></div>;
        case 'exams':
            const e = item as Exam;
            return (
                <div className="flex-1 mr-4">
                    <p className="font-bold text-sm text-gray-800 dark:text-gray-100">{e.subject}</p>
                    <p className="text-[10px] text-red-500 font-bold uppercase tracking-tight">{e.date} • {e.topic}</p>
                    {e.notes && <p className="text-[10px] text-gray-400 italic mt-1 line-clamp-1">{e.notes}</p>}
                </div>
            );
        case 'menu':
            const m = item as MenuItem;
            return <div><p className="text-[9px] font-bold text-orange-500 uppercase">{m.date}</p><p className="font-bold text-sm">{m.mainCourse}</p><p className="text-[9px] text-gray-400">Postre: {m.dessert}</p></div>;
        case 'events':
            const ev = item as SchoolEvent;
            return <div><p className="font-bold text-sm">{ev.name}</p><p className="text-[10px] text-amber-600 font-bold uppercase">{ev.date} • {ev.type}</p></div>;
        case 'centers':
            const c = item as Center;
            return <div><p className="font-bold text-sm">{c.name}</p></div>;
        case 'contacts':
            const ct = item as Contact;
            const centerName = centers.find(center => center.id === ct.centerId)?.name || 'Sin centro';
            return <div><p className="font-bold text-sm">{ct.name}</p><p className="text-[10px] text-gray-400 font-bold uppercase">{centerName} • {ct.phone}</p></div>;
        default: return null;
    }
}
