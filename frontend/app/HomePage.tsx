          ...
          case 'subjects':
              return (
                <InfoCard key={key} icon={<BookOpenIcon />} title="Clases de Hoy" index={index} onDragStart={onDragStart} onDragOver={onDragOver} onDragEnd={onDragEnd} isDragging={draggedIdx === index}>
                    ...
                    )) : <p className="text-center py-3 text-gray-400 text-sm font-medium">No hay clases hoy</p>}
                </InfoCard>
              );
          case 'contacts':
              return (
                    ...
                    {upcomingExams.length > 0 ? upcomingExams.map(e => (
                        <div key={e.id} className="p-3 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/30 flex justify-between items-center">
                            <div className="flex-1 mr-2">
                                ...
                                <p className="text-[10px] text-red-500 font-bold uppercase tracking-wide">{e.topic}</p>
                            </div>
    ...
    <div className="p-5 pb-24 dark:text-gray-100 animate-in fade-in duration-500">
      ...
      {activeModules.events && upcomingEvents.length > 0 && (
          <div className="mb-6 overflow-hidden transition-all duration-300">
              <button onClick={() => setIsEventsExpanded(!isEventsExpanded)} className={`w-full p-4 rounded-2xl flex items-center justify-between shadow-sm transition-all ${isEventsExpanded ? 'bg-amber-500 text-white rounded-b-none' : 'bg-amber-400 text-amber-900'}`}>
                  <div className="flex items-center">
                    <FlagIcon className={`w-5 h-5 mr-3 ${isEventsExpanded ? 'text-white' : 'text-amber-700'}`} />
                    <p className="font-bold text-sm">{isEventsExpanded ? 'Calendario de Eventos' : `Tienes ${upcomingEvents.length} eventos próximamente`}</p>
                  </div>
                  {isEventsExpanded ? <ChevronUpIcon className="w-5 h-5" /> : <ChevronDownIcon className="w-5 h-5" />}
              </button>
              {isEventsExpanded && (
                  <div className="bg-amber-50 dark:bg-gray-800 border-x border-b border-amber-200 dark:border-gray-700 rounded-b-2xl p-3 space-y-2">
